...
class AlphaSupabaseUser extends BaseAuthUser {
  AlphaSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> alphaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AlphaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
